...
console.group('2. Aprašykite funkcijų tipus prieš deklaruojant funkcijas');
{
  ...
  type TwoNumbersFunction = (num1: number, num2: number) => number;
  type OneNumberFunction = (number: number) => number;
  type StringFunction = (text: string) => void;


  const multiply: TwoNumbersFunction = (a, b) => {
    return a * b;
  };

  const power: TwoNumbersFunction = function (base, power) {
    return base ** power;
  };

  const squareRoot: OneNumberFunction = (number) => number ** (1 / 2);

  const root: TwoNumbersFunction = function (base, nthRoot) {
    return base ** (1 / nthRoot);
  }

  const printBlueText: StringFunction = (text) => console.log(`%c${text}`, 'color: #0000ee');
