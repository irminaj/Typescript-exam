...
console.group("Užduotys");
{
  ...
  console.group("2. Sukurkite ir tipais aprašykite funkciją, kuri sudarytų string'ą iš string'ų masyvo elementų pirmųjų raidžių");
  {
    ...
    */
    ...
    const additionalFunction = (abb: string, word: string) => abb + word[0];

    const createAbb = (words: string[]): string => {
      const abbreviation = words.reduce<string>(additionalFunction, "");
      return abbreviation;
    };
    ...
    arr.forEach((words: string[]) => console.log(`[${words.join(", ")}] -> ${createAbb(words)}`));
  }
  console.groupEnd();
