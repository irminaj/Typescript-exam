...
console.group("Užduotys");
{
    ...
    {
        const additionalFunction = (abb, word) => abb + word[0];
        const createAbb = (words) => {
            const abbreviation = words.reduce(additionalFunction, "");
            return abbreviation;
        };
        const arr = [
        ...
        arr.forEach((words) => console.log(`[${words.join(", ")}] -> ${createAbb(words)}`));
    }
    console.groupEnd();
