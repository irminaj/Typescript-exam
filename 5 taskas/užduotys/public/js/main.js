...
{
    const filterPeople = ({ name, surname, married }) => ({
        name,
        surname,
        married,
    });
    const result = people.map(filterPeople);
    console.log(result);
    console.log(people);
}
console.groupEnd();
...
{
    const mapPeopleMoney = ({ name, surname, income }) => ({
        name,
        surname,
        income,
    });
    const result2 = people.map(mapPeopleMoney);
    console.log(result2);
}
console.groupEnd();
console.groupCollapsed("5. Sukurtite masyvą su vardais, pavardėmis ir lytimi, pagal pradinį žmonių masyvą");
{
    const mapPeopleGender = ({ name, surname, sex }) => ({
        name,
        surname,
        sex,
    });
    const result3 = people.map(mapPeopleGender);
    console.log(result3);
}
console.groupEnd();
...
console.groupCollapsed("8. Atspausdinkite žmonių vardus ir pavardes, kurie turi mašinas");
{
    ...
    const peopleHasCars = people.filter(hasCar);
    const identities = peopleHasCars.map(createIdentity);
...
console.groupCollapsed("9. Atspausdinkite žmones kurie yra susituokę");
{
    ...
    const createIdentity = ({ name, surname }) => ({ name, surname });
    const peopleIsMarried = people.filter(isMarried);
    const identities = peopleIsMarried.map(createIdentity);
    console.log(identities);
}
console.groupEnd();
...
console.groupCollapsed('11. Performuokite žmonių masyvą, jog kiekvieno žmogaus savybė "income", taptų "salary"');
{
    ...
    const peopleSalary = people.map(convertToSalary);
